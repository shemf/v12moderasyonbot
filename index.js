...
client.on("message", async message => {
  const shka = message.content.toLocaleLowerCase();

  if (
    shka === "selam" ||
    shka === "sa" ||
    shka === "selamün aleyküm" ||
    shka === "selamun aleyküm" ||
    shka === "slm" ||
    shka === "sea"
  ) {
...
client.on('channelCreate', async channel => {
  const c = channel.guild.channels.get(db.fetch(`modlog_${channel.guild.id}`));
  if (!c) return;
...
client.on('emojiCreate', emoji => {
  const c = emoji.guild.channels.get(db.fetch(`modlog_${emoji.guild.id}`));
  if (!c) return;
...
client.on('emojiUpdate', (oldEmoji, newEmoji) => {
  const c = newEmoji.guild.channels.get(db.fetch(`modlog_${newEmoji.guild.id}`));
  if (!c) return;
...
client.on('guildBanAdd', async (guild, user) => {    
    const channel = guild.channels.get(db.fetch(`modlog_${guild.id}`));
  if (!channel) return;
...
client.on('messageDelete', async message => {    
  if(message.author.bot) return
    ...
    const channel = message.guild.channels.get(db.fetch(`modlog_${message.guild.id}`));
  if (!channel) return;
...
client.on('messageUpdate', async(oldMessage, newMessage) => {
    ...
    const channel = oldMessage.guild.channels.get(db.fetch(`modlog_${oldMessage.guild.id}`));
    if(!channel) return;
...
client.on('roleCreate', async (role) => {    
    ...
    const channel = role.guild.channels.get(db.fetch(`modlog_${role.guild.id}`));
  if (!channel) return;
...
client.on('voiceStateUpdate', (oldMember, newMember) => {
 ...
 // if (!logA[oldMember.guild.id]) return;
  ...
  if (db.has(`modlog_${oldMember.guild.id}`) === false) return;
  
  var kanal = oldMember.guild.channels.get(db.fetch(`modlog_${oldMember.guild.id}`).replace("<#", "").replace(">", ""))
  if (!kanal) return;
